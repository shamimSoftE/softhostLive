...
{   
    ...
    public function about_section_update(Request $request, $id){
        ...
        $request->validate([
            ...
            'about_tiitle_1' => 'required|max:250',
			'about_tiitle_2' => 'required|max:250',
			'about_tiitle_3' => 'required|max:250',
			'sucess_rate' => 'required|max:250',
			'satisfied_client' => 'required|max:250',
			'about_intro_video' => 'required|max:250',
            'about_experince_yers' => 'required|numeric',
            'about_text' => 'required',
            'about_image' => 'mimes:jpeg,jpg,png',
			'about_image1' => 'mimes:jpeg,jpg,png',
			'about_image2' => 'mimes:jpeg,jpg,png',
        ]);

        $st = Sectiontitle::where('language_id', $id)->first();

        if($request->hasFile('about_image1')){
            @unlink('assets/front/img/'. $st->about_image1);
            $file = $request->file('about_image1');
            $extension = $file->getClientOriginalExtension();
            $about_image1 = time().rand().'.'.$extension;
            $file->move('assets/front/img/', $about_image1);

            $st->about_image1 = $about_image1;
        }
		
		
		
		        if($request->hasFile('about_image2')){
            @unlink('assets/front/img/'. $st->about_image2);
            $file = $request->file('about_image2');
            $extension = $file->getClientOriginalExtension();
            $about_image2 = time().rand().'.'.$extension;
            $file->move('assets/front/img/', $about_image2);

            $st->about_image2 = $about_image2;
        }
		...
		$st->about_tiitle_1 = $request->about_tiitle_1;
		$st->about_tiitle_2 = $request->about_tiitle_2;
		$st->about_tiitle_3 = $request->about_tiitle_3;
		$st->sucess_rate = $request->sucess_rate;
		$st->satisfied_client = $request->satisfied_client;
